...
def train_cli():
    ...
    parser.add_argument('--prot-proj', default="agg", choices=["avg","agg","transformer"], help="Change the protein projector method")
...
def train(
    ...
    # Load model
    print(238, f'train, config.prot_proj : {config.prot_proj}')
    ...
    if not config.no_wandb:
        wandb_logger = WandbLogger(project=config.wandb_proj, name=f"bio_focal_a-1_g2_coeff=0.25_ce*1_info_0.25", log_model=False)
        #wandb_logger = WandbLogger(project=config.wandb_proj, name=f"biosnap_focal_a-1_g2_coeff=1_ce*1", log_model=False)
        #wandb_logger = WandbLogger(project=config.wandb_proj, name=f"davis_focal_a-1_g2_coeff=1_ce*1", log_model=False)
    ...
    if args.eval_pcba:
        callbacks.append(PCBAEvaluationCallback(eval_interval=20))
...
def test_cli():
    ...
    parser.add_argument("--num-heads-agg", type=int, default=4, help="Number of attention heads for learned aggregation", dest="num_heads_agg")
    parser.add_argument("--dropout", type=float, help="Dropout rate for transformer", dest="dropout")
    parser.add_argument("--batch-size", type=int, default=1024, help="batch size for training/val/test")
    parser.add_argument("--num-workers", type=int, default=0, help="number of workers for intial data processing and dataloading during training")
    parser.add_argument("--no-wandb", action="store_true", help="Do not use wandb")
    parser.add_argument("--model-size", default="small", choices=["small", "large"], help="Choose the size of the model")
    parser.add_argument("--ship-model", help="Train a final to ship model, while excluding the uniprot id's specified by this argument.", dest="ship_model")
